"use client";

import { motion } from "framer-motion";
import { ExternalLink, Code, Mail, Globe } from "lucide-react";
import { useState } from "react";
import { useGlobal } from "@/components/core/GlobalProvider";

export default function SocialBar3D() {
    const { t } = useGlobal();
    const [clickedId, setClickedId] = useState<string | null>(null);

    const socials = [
        {
            id: "github",
            label: "GitHub",
            href: t("contact.github"),
            svg: <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" /></svg>
        },
        {
            id: "instagram",
            label: "Instagram",
            href: t("contact.instagram"),
            svg: <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.058-1.69-.072-4.949-.072zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" /></svg>
        },
        {
            id: "email",
            label: "Email",
            href: `mailto:${t("contact.email")}`,
            svg: <Mail className="w-4 h-4" />
        },
    ];

    const handleClick = (id: string) => {
        setClickedId(id);
        setTimeout(() => setClickedId(null), 1000);
    };

    return (
        <motion.div
            initial={{ opacity: 0, x: 40 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 1.2, delay: 2, ease: "easeOut" }}
            className="fixed right-6 top-1/2 -translate-y-1/2 z-[100] flex flex-col gap-3"
        >
            {socials.map(({ svg, href, label, id }, i) => (
                <motion.a
                    key={label}
                    href={href}
                    target="_blank"
                    rel="noopener noreferrer"
                    aria-label={label}
                    onClick={() => handleClick(id)}
                    initial={{ opacity: 0, x: 20 }}
                    animate={{
                        opacity: 1,
                        x: clickedId === id ? 100 : 0,
                    }}
                    transition={{
                        x: { type: "spring", stiffness: 300, damping: 20 },
                        opacity: { duration: 0.6, delay: 2.2 + i * 0.15 }
                    }}
                    className="group relative w-11 h-11 flex items-center justify-center rounded-xl bg-white/5 border border-white/10 backdrop-blur-xl text-gray-400 hover:bg-accent/20 hover:border-accent/40 hover:text-accent hover:shadow-[0_0_20px_rgba(124,58,237,0.3)] transition-all duration-300"
                >
                    {svg}

                    {/* Tooltip */}
                    <span className="absolute right-full mr-3 px-2 py-1 text-[10px] font-mono text-white/70 bg-black/80 border border-white/10 rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap pointer-events-none">
                        {label}
                    </span>
                </motion.a>
            ))}

            {/* Vertical line decoration */}
            <div className="w-px h-16 bg-gradient-to-b from-white/20 to-transparent mx-auto mt-2" />
        </motion.div>
    );
}

